...
export const markTaskCompleted = async (req: AuthenticatedRequest, res: Response): Promise<Response> => {
  try {
    if (!req.user?._id) {
      return res.status(401).json({ msg: "Unauthorized" });
    }
...
export const generateAITask = async (
  ...
  try {
    ...
    const { recentDistance = 0, streakDays = 0, avgSpeed = 0 } = req.body as GenerateAITaskBody;
...
`;
    ...
    let aiTask: AITaskPayload | null = null;
    let attempts = 0;
    ...
    do {
      ...
      try {
        const parsed = JSON.parse(cleanedMessage) as Partial<AITaskPayload>;
        if (parsed.description && parsed.difficulty && parsed.type && typeof parsed.target === "number") {
          aiTask = parsed as AITaskPayload;
        }
      } catch {
      ...
      attempts < 2 &&
      (!aiTask || recentTasks.some((t: ChallengeDocument) => t.description === aiTask?.description))
    );
...
export const getChallenges = async (_req: AuthenticatedRequest, res: Response): Promise<Response> => {
  try {
...
export const updateChallengeProgress = async (
  ...
  try {
    const { challengeId, steps, distance } = req.body as UpdateChallengeProgressBody;
